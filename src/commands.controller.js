...
async function handleError(interaction, command, args, message, error) {
    ...
    const errorResponse = { content: `\`${command} ${args || ''}\``, embeds: [errorEmbed]/* , ephemeral: true */ };
    if (interaction) {
...
async function commandsController(VoiceControl, message = null, interaction = null) {
    let command;
    let args;
    ...
    if (message && !interaction) {
        ...
        fakeInteraction.voiceChannel = await voiceUtils.getUserCurrentChannelFromMsg(message);
    ...
    try {
        ...
        switch (command) {
            case 'anison':
                response = await commands.get('anison').execute(fakeInteraction, VoiceControl);
                break;
            case 'clear':
                response = await commands.get('clear').execute(fakeInteraction, VoiceControl);
                break;
            case 'help':
            ...
            case 'join':
                ...
                break;
            case 'leave':
                response = await commands.get('leave').execute(fakeInteraction, VoiceControl);
                break;
            case 'music':
                response = await commands.get('sound').execute(fakeInteraction, args, 'music', VoiceControl);
                break;
            case 'pause':
            ...
            case 'ping':
                ...
                break;
            case 'play':
                response = await commands.get('play').execute(fakeInteraction, args, VoiceControl);
                break;
            case 'queue':
            ...
            case 'roll':
                ...
                break;
            case 'savelist':
                response = await commands.get('savelist').execute(fakeInteraction, args);
                break;
            case 'shutdown':
            ...
            case 'skip':
                ...
                break;
            case 'sound':
                response = await commands.get('sound').execute(fakeInteraction, args, 'sound', VoiceControl);
                break;
            case 'soundlist':
                response = await commands.get('soundlist').execute(fakeInteraction, args);
                break;
            case 'stop':
