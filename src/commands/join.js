...
const name = 'join';
const description = 'Rejoins le salon vocal (nom ou id). Si aucun salon n\'est précisé : rejoins le salon de l`utilisateur';
...
module.exports = {
    ...
    async execute(interaction, args) {
        ...
        if (args.length > 0) {
            channelToJoin = interaction.guild.channels.cache.find(element => (element.name.includes(args[0])) || element.id.includes(args[0]));
